...
const CategoryDetailScreen = () => {
  ...
  useEffect(() => {
    if (categoryName) {
      fetchCategoryDetail(decodeURIComponent(categoryName));
      if (session?.user) {
        loadSavedOffers();
      }
    }
  ...
  const fetchCategoryDetail = async (name: string) => {
    try {
      setIsLoading(true);
      console.log('Fetching offers for category:', name);
      ...
      console.log('Category offers query result:', { offers, error });
      ...
      if (offers && offers.length > 0) {
        ...
        console.log('No offers found for category:', name);
        setCategoryDetail({
          name,
          totalOffers: 0,
          stores: [],
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-pink-50/50 ${isMobile ? 'pb-16' : 'pt-20'}`}>
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className={`text-gray-600 ${isMobile ? 'text-sm' : 'text-base'}`}>
                    {categoryDetail.totalOffers} active offers
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className={`${isMobile ? 'p-4 space-y-6' : 'p-8 w-full space-y-8'}`}>
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-purple-200/60">
            <CardContent className="p-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-900">{categoryDetail.totalOffers}</p>
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-emerald-200/60">
            <CardContent className="p-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-900">{categoryDetail.stores.length}</p>
            ...
            <CardContent className="p-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Category Rating</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        {/* Available Stores */}
        {categoryDetail.stores.length > 0 && (
          <Card className="bg-white/80 backdrop-blur-sm border-gray-200/60">
            ...
            <CardContent>
              <div className="flex flex-wrap gap-2">
                {categoryDetail.stores.map((store, index) => (
                  <Badge
                    key={index}
                    variant="secondary"
        ...
        <Card className="bg-white/80 backdrop-blur-sm border-gray-200/60">
          ...
          <CardContent>
            {categoryDetail.offers.length === 0 ? (
              <div className="text-center py-12">
                <FolderOpen className="w-16 h-16 text-gray-300 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-gray-900 mb-2">No offers available</h3>
                <p className="text-gray-500">There are no active offers in this category at the moment.</p>
              </div>
            ) : (
              <div className={`grid gap-6 ${isMobile ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3'}`}>
                {categoryDetail.offers.map((offer) => (
                  <Card
                    key={offer.lmd_id}
