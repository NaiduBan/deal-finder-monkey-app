...
const BrandDetailScreen = () => {
  ...
  const fetchBrandDetail = async (name: string) => {
    try {
      setIsLoading(true);
      console.log('Fetching offers for brand:', name);
      
      // Search in both store and categories fields for brand names
      const { data: offers, error } = await supabase
        .from('Offers_data')
        .select('lmd_id, title, description, image_url, offer_value, type, code, end_date, store, categories')
        .or(`store.ilike.%${name}%,categories.ilike.%${name}%`);

      console.log('Brand offers query result:', { offers, error });
      ...
      if (offers && offers.length > 0) {
        ...
        const filteredOffers = offers.filter(offer => {
          if (isOfferActive(offer.end_date)) {
            // Check if brand name matches store or is in categories
            const matchesStore = offer.store && offer.store.toLowerCase().includes(name.toLowerCase());
            const matchesCategory = offer.categories && 
              offer.categories.split(',').some(cat => 
                cat.trim().toLowerCase().includes(name.toLowerCase())
              );
            
            if ((matchesStore || matchesCategory) && offer.store) {
        ...
        console.log('No offers found for brand:', name);
        setBrandDetail({
          name,
          totalOffers: 0,
          stores: [],
    ...
    <div className={`min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-pink-50/50 ${isMobile ? 'pb-16' : 'pt-20'}`}>
      ...
      <div className={`${isMobile ? 'p-4 space-y-6' : 'p-8 max-w-7xl mx-auto space-y-8'}`}>
        ...
        <Card className="bg-white/80 backdrop-blur-sm border-gray-200/60">
          ...
          <CardContent>
            {brandDetail.offers.length === 0 ? (
              <div className="text-center py-12">
                <Star className="w-16 h-16 text-gray-300 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-gray-900 mb-2">No offers available</h3>
                <p className="text-gray-500">There are no active offers for this brand at the moment.</p>
              </div>
            ) : (
